...
#[tokio::test]
async fn test_server_from_acceptor() {
    let _ = pretty_env_logger::try_init();

    let bind_addr: SocketAddr = "127.0.0.1:0".parse().unwrap();
    let acceptor = hyper::server::conn::AddrIncoming::bind(&bind_addr).unwrap();
    let local_addr = acceptor.local_addr();

    // Setup a server to expect a single GET /foo request.
    let server = httptest::Server::from_acceptor(acceptor, local_addr, http::uri::Scheme::HTTP);
    server.expect(
        Expectation::matching(all_of![request::method("GET"), request::path("/foo")])
            .respond_with(status_code(200)),
    );
